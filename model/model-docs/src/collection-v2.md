...
The current version of the Project Locations Model differentiates between **mandatory and non-mandatory fields**. Mandatory fields are indicated in the field descriptions (see Annex 1 and 2).  
...
We highlight two common cases for generating the required data: 
 ...
 1. **Desktop-production**: For manual data collection, we recommend using the [Excel Templates](./annex1.md) to store all attributes and merging this information with spatial information (feature geometries) in a GIS software. Since the Excel Templates are pre-configured, it is easier to provide data that is compliant with this [data-model](./annex2.md) and pass the [validation check](./validation.md) as a way to ensure data quality. A short [video tutorial](www.youtube.com) demonstrates how to use the Excel template with QGIS software.
